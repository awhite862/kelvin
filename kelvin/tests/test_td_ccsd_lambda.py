...
class TDCCSDLambdaTest(unittest.TestCase):
    ...
    def test_Be_rk4_omega(self):
        ...
        tdccsdT = TDCCSD(sys, prop, T=T, mu=mu, ngrid=85)
    ...
    def test_Be_omega_active(self):
        ...
        tdccsdT = TDCCSD(sys, prop, T=T, mu=mu, ngrid=100,
                         athresh=1e-20, iprint=0, saveT=True)
        Eref, Eccref = tdccsdT.run()
        Eout, Eccout = tdccsdT._ccsd_lambda()
        diff = abs(Eccref - Eccout)
        error = "Expected: {}  Actual: {}".format(Eccref, Eccout)
        self.assertTrue(diff < 1e-10, error)
    ...
    def test_Be_rk1(self):
        ...
        ccsdT = ccsd(sys, T=T, mu=mu, ngrid=25, iprint=0, quad="mid")
    ...
    def test_Be_rk4(self):
        ...
        # compute \bar{Lambda} at \tau = 0
        ccsdT = ccsd(sys, T=T, mu=mu, ngrid=280, iprint=0, quad="mid")
        Eref, Eccref = ccsdT.run()
        ccsdT._ft_ccsd_lambda()
        ng = ccsdT.ngrid
        L1 = ccsdT.L1
    ...
    def test_Be_cn(self):
        ...
        # compute \bar{Lambda} at \tau = 0
        ccsdT = ccsd(sys, T=T, mu=mu, ngrid=300, iprint=0, quad="mid")
        Eref, Eccref = ccsdT.run()
        ccsdT._ft_ccsd_lambda()
        ng = ccsdT.ngrid
        L1 = ccsdT.L1
    ...
    def test_Be_rk124(self):
        ...
        prop = {"tprop": "rk4", "lprop": "rk4"}
        tdccsdT4 = TDCCSD(sys, prop, T=T, mu=mu, ngrid=320, saveT=True)
        Eout, Eccout = tdccsdT4.run()
        Etmp, Ecctmp = tdccsdT4._ccsd_lambda()
        prop = {"tprop": "rk2", "lprop": "rk2"}
        tdccsdT2 = TDCCSD(sys, prop, T=T, mu=mu, ngrid=2400, saveT=True)
        Eout, Eccout = tdccsdT2.run()
        Etmp, Ecctmp = tdccsdT2._ccsd_lambda()
        prop = {"tprop": "rk1", "lprop": "rk1"}
        tdccsdT1 = TDCCSD(sys, prop, T=T, mu=mu, ngrid=2400, saveT=True)
        Eout, Eccout = tdccsdT1.run()
        Etmp, Ecctmp = tdccsdT1._ccsd_lambda()
        d1_14 = numpy.linalg.norm(tdccsdT4.L1 - tdccsdT1.L1)\
            / numpy.linalg.norm(tdccsdT4.L1)
        d2_14 = numpy.linalg.norm(tdccsdT4.L2 - tdccsdT1.L2)\
            / numpy.linalg.norm(tdccsdT4.L2)
        ...
        d1_24 = numpy.linalg.norm(tdccsdT4.L1 - tdccsdT2.L1)\
            / numpy.linalg.norm(tdccsdT4.L1)
        d2_24 = numpy.linalg.norm(tdccsdT4.L2 - tdccsdT2.L2)\
            / numpy.linalg.norm(tdccsdT4.L2)
    ...
    def test_Be_u_vs_g(self):
        ...
        d1a = numpy.linalg.norm(l1aref - tdccsdT.L1[0])\
            / numpy.linalg.norm(l1aref)
        d1b = numpy.linalg.norm(l1bref - tdccsdT.L1[1])\
            / numpy.linalg.norm(l1bref)
        d2aa = numpy.linalg.norm(l2aaref - tdccsdT.L2[0])\
            / numpy.linalg.norm(l2aaref)
        d2ab = numpy.linalg.norm(l2abref - tdccsdT.L2[1])\
            / numpy.linalg.norm(l2abref)
        d2bb = numpy.linalg.norm(l2bbref - tdccsdT.L2[2])\
            / numpy.linalg.norm(l2bbref)
    ...
    def test_Be_ccd(self):
        ...
        # compute \bar{Lambda} at \tau = 0
        ccsdT = ccsd(
            sys, T=T, mu=mu, ngrid=140, iprint=0, quad="mid", singles=False)
        Eref, Eccref = ccsdT.run()
        ccsdT._ft_ccsd_lambda()
        ng = ccsdT.ngrid
        L2 = ccsdT.L2
